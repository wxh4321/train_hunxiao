...
function start_run()
{
    ...
    $.ajax({
        ...
        function run_antidebug_code(table,n_a,para1,para2,init_key) {
                ...
                if(result1[1]){
                    if (index===1){
                        ...
                        if(s===init_key){
                            ...
                            return run_normal_code(table,[result[0],result[1],result1[0]],para1,para2);
                        }
                    }
                    code +=get_code(table,result[0],result[1]);
                    return run_antidebug_code(table,[result[0],result[1],result1[0]],para1,para2,init_key);
                }
                if (index!==1)
                {
                    init_destory_area();
                    return run_self_destory_code(table,[result[0],result[1],result1[0]],para1,para2);
                }
            });

        }

        function run_normal_code(table,n_a,para1,para2) {
            var result = g_n_i(n_a,para1,para2);
            index+=1;
            port = ":8082";
            $.ajax({
        ...
        function run_self_destory_code(table,n_a,para1,para2) {
            var result = g_n_i(n_a,para1,para2);
            port = ":8082";
            $.ajax({
                url:url+port,
        ...
        run_antidebug_code(table,init_arr,para1,para2,init_key);//document.write引起的菊花
    });
